...
class PASTIS_Dataset(Dataset):
    def __init__(
        ...
        """Pytorch Dataset class to load samples from the PASTIS dataset, for semantic and panoptic
        segmentation. The Dataset yields ((data, dates), target) tuples, where:
        ...
        if mono_date is not None:
            self.mono_date = (
                datetime(*map(int, mono_date.split("-"))) if "-" in mono_date else int(mono_date)
            )
        self.memory = {}
        self.memory_dates = {}
        self.class_mapping = (
            np.vectorize(lambda x: class_mapping[x]) if class_mapping is not None else class_mapping
        )
        self.target = target
        self.sats = sats
    ...
    def __getitem__(self, item):
        ...
        if self.mono_date is not None:
            if isinstance(self.mono_date, int):
                ...
                mono_date = {s: int((dates[s] - mono_delta).abs().argmin()) for s in self.sats}
        ...
        sample = {"image": data, "label": target, "dates": dates}
...
def prepare_dates(date_dict, reference_date):
    ...
    d = d[0].apply(
        lambda x: (
            datetime(int(str(x)[:4]), int(str(x)[4:6]), int(str(x)[6:])) - reference_date
        ).days
    )
    return d.values
