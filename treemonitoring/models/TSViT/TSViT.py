...
from treemonitoring.models.TSViT.module import Attention, FeedForward, PreNorm
...
class STViT(nn.Module):
    ...
    def __init__(self, model_config):
        ...
        self.space_transformer = Transformer(
            self.dim, self.depth, self.heads, self.dim_head, self.dim * self.scale_dim, self.dropout
        )
        ...
        self.mlp_head = nn.Sequential(
            nn.LayerNorm(self.dim), nn.Linear(self.dim, self.num_classes * self.patch_size**2)
        )
    ...
    def forward(self, x):
        ...
        cls_temporal_tokens = repeat(
            self.temporal_token, "() () d -> b t k d", b=b, t=1, k=self.num_patches_1d**2
        )
...
class TSViT_single_token(nn.Module):
    ...
    def __init__(self, model_config):
        ...
        self.to_patch_embedding = nn.Sequential(
            Rearrange(
                "b t c (h p1) (w p2) -> (b h w) t (p1 p2 c)", p1=self.patch_size, p2=self.patch_size
            ),
            nn.Linear(patch_dim, self.dim),
        )
...
class TSViT_static_position_encodings(nn.Module):
    """Temporal-Spatial ViT with static (no lookup) position encodings (used in ablation, section
    4.2)"""
    ...
    def forward(self, x):
        ...
        x = x.reshape(B, self.num_classes, self.num_patches_1d**2, self.patch_size**2).permute(
            0, 2, 3, 1
        )
        x = x.reshape(B, H, W, self.num_classes)
        x = x.permute(0, 3, 1, 2)
...
class TSViT_global_attention_spatial_encoder(nn.Module):
    """Temporal-Spatial ViT where spatial encoder attends to all cls tokens (used in ablation,
    section 4.2)"""
...
class TSViT(nn.Module):
    """Temporal-Spatial ViT5 (used in main results, section 4.3) For improved training speed, this
    implementation uses a (365 x dim) temporal position encodings indexed for each day of the year.

    Use TSViT_lookup for a slower, yet more general implementation of lookup position encodings
    """
...
class TSViT_lookup(nn.Module):
    """Temporal-Spatial ViT5 (used in main results, section 4.3) This is a general implementation of
    lookup position encodings for all dates found in a training set.

    During inference, position encodings are calculated via linear interpolation between dates found
    ...
    def __init__(self, model_config, train_dates):
        ...
        self.train_dates = torch.nn.Parameter(
            data=torch.tensor(train_dates), requires_grad=False
        )  # .cuda()
        ...
        self.temporal_pos_embedding = nn.Parameter(
            torch.randn(len(self.train_dates), self.dim), requires_grad=True
        )
    ...
    def forward(self, x, inference=False):
        ...
        if inference:
            self.update_inference_temporal_position_embeddings()
            temporal_pos_embedding = self.get_inference_temporal_position_embeddings(xt).to(
                x.device
            )
        else:
    ...
    def update_inference_temporal_position_embeddings(self):
        ...
        for i, evdate in enumerate(self.eval_dates):
            if evdate < min_val:
                ...
                pos_eval[i] = (upper_date - evdate) / (
                    upper_date - lower_date
                ) * self.temporal_pos_embedding[lower_idx] + (evdate - lower_date) / (
                    upper_date - lower_date
...
if __name__ == "__main__":
    ...
    train_config = {
        "dataset": "psetae_repl_2018_100_3",
