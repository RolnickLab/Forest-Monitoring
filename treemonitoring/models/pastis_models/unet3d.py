"""Taken from https://github.com/roserustowicz/crop-type-mapping/ Implementation by the authors of
the paper : "Semantic Segmentation of crop type in Africa: A novel Dataset and analysis of deep
learning methods" R.M. Rustowicz et al.
...
class UNet3D(nn.Module):
    def __init__(self, in_channel, n_classes, feats=8, pad_value=None, zero_pad=True):
        ...
        self.en3 = conv_block(3, feats * 4, feats * 4)
    ...
    def forward(self, x, batch_positions=None):
        ...
        if self.pad_value is not None:
            if pad_mask.any():
                ...
                pad_mask = pad_mask[
                    :, : final.shape[-1]
                ]  # match new temporal length (due to pooling)
                pad_mask = ~pad_mask  # 0 on padded values
                out = (final.permute(1, 2, 3, 0, 4) * pad_mask[None, None, None, :, :]).sum(
                    dim=-1
